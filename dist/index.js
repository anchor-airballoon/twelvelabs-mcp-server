...
// 도구 목록
const CREATE_INDEX_TOOL = {
    name: "create_index",
    description: "Creates a new index in TwelveLabs. Useful before uploading videos or performing searches. " +
...
const LIST_INDEXES_TOOL = {
    name: "list_indexes",
    description: "Lists all indexes in your TwelveLabs account. " +
...
const GET_INDEX_TOOL = {
    name: "get_index",
    description: "Retrieves details of a specific index. " +
...
const UPDATE_INDEX_TOOL = {
    name: "update_index",
    description: "Updates the name of a specific index. " +
...
const DELETE_INDEX_TOOL = {
    name: "delete_index",
    description: "Deletes a specific index and all videos within it. This action cannot be undone. " +
        "Input: { indexId: string }",
    inputSchema: {
        type: "object",
        properties: {
...
const LIST_TASKS_TOOL = {
    name: "list_tasks",
    description: "Lists all video indexing tasks in your account. " +
...
const GET_TASK_TOOL = {
    name: "get_task",
    description: "Retrieves details of a specific video indexing task. " +
        "Input: { taskId: string }",
    inputSchema: {
        type: "object",
        properties: {
...
const DELETE_TASK_TOOL = {
    name: "delete_task",
    description: "Deletes a specific video indexing task. You can only delete tasks with status 'ready' or 'failed'. " +
        "Input: { taskId: string }",
    inputSchema: {
        type: "object",
        properties: {
...
const UPLOAD_VIDEOS_TOOL = {
    name: "upload_videos",
    description: "Uploads videos to an existing TwelveLabs index from direct URLs or local file paths. " +
...
const SEARCH_VIDEOS_TOOL = {
    name: "search_videos",
    description: "Searches videos within a specific index using text queries. " +
...
const GENERATE_TEXT_TOOL = {
    name: "generate_text",
    description: "Generates text (transcription or summary) from a specified video. " +
...
const GENERATE_GIST_TOOL = {
    name: "generate_gist",
    description: "Generates titles, topics, and hashtags for your videos. " +
...
const GENERATE_SUMMARY_TOOL = {
    ...
    description: "Generates summaries, chapters, or highlights for your videos. " +
...
const GET_IMPORT_STATUS_TOOL = {
    name: "get_import_status",
    description: "Retrieves the current status for each video from a specified integration and index. " +
...
const GET_IMPORT_LOGS_TOOL = {
    name: "get_import_logs",
    description: "Retrieves the import logs for a specified integration. " +
...
const IMPORT_VIDEOS_TOOL = {
    name: "import_videos",
    description: "Imports videos from an integration into an index. " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "list_indexes": {
                const { page, pageLimit, sortBy, sortOption, indexName } = args;
            ...
            case "upload_videos": {
                const { indexId, url, filePath, enableVideoStream = true } = args;
                if (!url && !filePath) {
                    return {
            ...
            case "list_tasks": {
                const { page, pageLimit, sortBy, sortOption, indexId, status } = args;
...
async function uploadVideos(indexId, url, filePath, enableVideoStream = true) {
    try {
        // API 엔드포인트
...
async function searchVideos(indexId, queryText, options = ["visual", "audio"], operator = "or", limit = 10) {
    try {
...
async function generateTextFromVideo(videoId, mode = "transcript", prompt, temperature = 0.2) {
    try {
        // 엔드포인트 확인
...
// 새로 추가한 함수들
async function listIndexes(page, pageLimit, sortBy, sortOption, indexName) {
    try {
        // 쿼리 파라미터 구성
        ...
        if (pageLimit)
            queryParams.append('page_limit', pageLimit.toString());
        if (sortBy)
            queryParams.append('sort_by', sortBy);
        if (sortOption)
            queryParams.append('sort_option', sortOption);
        if (indexName)
            queryParams.append('index_name', indexName);
...
async function getIndex(indexId) {
    try {
...
async function updateIndex(indexId, indexName) {
    try {
        const url = `${BASE_URL}/indexes/${indexId}`;
        const body = {
            index_name: indexName
...
async function listTasks(page, pageLimit, sortBy, sortOption, indexId, status) {
    try {
        // 쿼리 파라미터 구성
        const queryParams = new URLSearchParams();
        if (page)
...
async function generateGist(videoId, types) {
    try {
...
async function generateSummary(videoId, type, prompt, temperature = 0.2) {
    try {
...
// 새로운 함수들 추가
async function importVideos(integrationId, indexId, incrementalImport = true, retryFailed = false) {
    try {
...
async function getImportStatus(integrationId, indexId) {
    try {
...
async function getImportLogs(integrationId) {
    try {
